...
export const ThemeCustomizationDrawer: React.FC = () => {
  ...
  useEffect(() => {
    if (drawerOpen) {
      document.documentElement.classList.add('customizer-drawer-open');
      document.body.classList.add('customizer-drawer-open');
    }
  }, [drawerOpen]);

  const handleAfterOpenChange = (open: boolean) => {
    if (!open) {
      document.documentElement.classList.remove('customizer-drawer-open');
      document.body.classList.remove('customizer-drawer-open');
    }
  };
      ...
      <div className="customizer-drawer-body">
        ...
        {activeTab === 'palette' && (
          <div className="fade-in">
            ...
            {/* BUTTON CORNER SHAPE */}
            <div className="customizer-section-block">
              <div className="customizer-section-title">BUTTON CORNER SHAPE</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {BUTTON_SHAPE_OPTIONS.map((shape) => {
                  const isActive = buttonBorderRadius === shape.radius;
                  return (
                    <div
                      key={shape.id}
                      className={`dual-option-card ${isActive ? 'active' : ''}`}
                      ...
                      onClick={() => dispatch(setButtonBorderRadius(shape.radius))}
                    >
                      {isActive && <CheckOutlined style={{ marginRight: 4, color: presetColor }} />}
                      {shape.name}
                    </div>
            ...
            {/* ERROR DISPLAY STYLE */}
            <div className="customizer-section-block">
              <div className="customizer-section-title">ERROR DISPLAY STYLE</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                {ERROR_STYLE_OPTIONS.map((opt) => {
                  const isActive = errorStyle === opt.id;
                  return (
                    <div
                      key={opt.id}
                      className={`dual-option-card ${isActive ? 'active' : ''}`}
                      ...
                      onClick={() => dispatch(setErrorStyle(opt.id as ErrorStyle))}
                    >
                      {isActive && (
                        <CheckOutlined
                          style={{
